...
sapply(combi,class)
# 
# names = sapply(names(combi), FUN=function(x){length(levels(combi[,x]))})
# continuous = names(names[names>=12])
# categorical = names(names[names<12])
# 
# combi <- rbind(train, test)
...
combi$cutSrchScore = cut2(combi$srch_query_affinity_score, g=10)
...
summary(combi$srch_query_affinity_score*-1)
...
gbmGrid <-  expand.grid(interaction.depth = c(1,5,9,14),
                        n.trees = (1:8)*200,
                        shrinkage = c(.01, .05, .1))
